...
class Application:
    def __init__(self, dataPath = "./output/"):
        ...
        """
        # initialize webcam capture, get length and width of frame
        ...
        self.ACCESS_KEY = "AKIASWSXDFK6G4BSUQ63" 
        self.SECRET_KEY = "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"
    ...
    def horizontalScan(self):
        """
            Scans for a face moving horizontally left to right. Stops when a face is detected
        """
        if self.iVer != 1.5:
            self.iVer = 1.5
    ...
    def videoLoop(self):
        ...
        if ref:
            ...
            if self.rec == True:    # started to recognize
                ...
                # check if within a minute interval and send the text
                if (abs(self.now - datetime.now().minute) >= 1):
